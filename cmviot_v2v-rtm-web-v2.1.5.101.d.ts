...
/** @zh-cn
 * SDK 登录 Cmviot RTM 系统成功。
 */
/**
 * The SDK has logged in the Cmviot RTM system.
 */
 | 'LOGIN_SUCCESS'
/** @zh-cn
 * SDK 登录 Cmviot RTM 系统失败。
 */
/**
 * The SDK fails to log in the Cmviot RTM system, because, for example, the token has expired.
 */
 | 'REJECTED_BY_SERVER'
/** @zh-cn
 * SDK 无法登录 Cmviot RTM 系统超过 6 秒，停止登录。
 */
/**
 ...
 */
 | 'LOST'
/** @zh-cn
 * SDK 与 Cmviot RTM 系统的连接被中断超过 4 秒。
 */
/**
 * The connection between the SDK and the Cmviot RTM system is interrupted for more than four seconds.
 */
 | 'INTERRUPTED'
/** @zh-cn
 * SDK 已登出 Cmviot RTM 系统。
 */
/**
 * The SDK has logged out of the Cmviot RTM system.
 */
 | 'LOGOUT'
/** @zh-cn
 * SDK 被服务器禁止登录 Cmviot RTM 系统。
 */
/**
 * Login is banned by the Cmviot RTM server.
 */
/** @zh-cn
 * 另一个用户正以相同的 uid 登陆 Cmviot RTM 系统。
 */
/**
 * Another instance has logged in the Cmviot RTM system with the same uid.
 */
 | 'SAME_UID_LOGIN'
/** @zh-cn
 * 用户使用的token已过期。
 ...
 */
 | 'TOKEN_EXPIRED';
/** @zh-cn
 * SDK 与 Cmviot RTM 系统的连接状态类型
 */
/**
 * Connection states between the SDK and the Cmviot RTM system.
 */
declare type ConnectionState = 
/** @zh-cn
 * 初始状态。SDK 未连接到 Cmviot RTM 系统。
 *
 * App 调用方法 {@link RTMClient.login} 时，SDK 开始登录 Cmviot RTM 系统，触发回调 {@link RTMEvents.status}，并切换到 {@link CONNECTING} 状态。
 *
 */
/**
 * When the app calls the {@link RTMClient.login} method, the SDK logs in the Cmviot RTM system, triggers the {@link RTMEvents.status} callback, and switches to the {@link CONNECTING} state.
 */
'DISCONNECTED'
/** @zh-cn
 * SDK 正在登录 Cmviot RTM 系统。
 *
 ...
 */
 | 'CONNECTING'
/** @zh-cn
 * SDK 已登录 Cmviot RTM 系统。
 * <ul>
 *     <li>如果 SDK 与 Cmviot RTM 系统的连接由于网络问题中断，SDK 会触发回调 {@link RTMEvents.status}，并切换到 {@link RECONNECTING} 状态。</li>
 *     <li>如果 SDK 因为相同 ID 已在其他实例或设备中登录等原因被服务器禁止登录，会触发回调 {@link RTMEvents.status}，并切换到 {@link FAILED} 状态。</li>
 *     <li>如果 App 调用方法 {@link RTMClient.logout}，SDK 登出 Cmviot RTM 系统成功，会触发回调 {@link RTMEvents.status}，并切换到 {@link DISCONNECTED} 状态。</li>
 * </ul>
 */
/**
 * The SDK has logged in the Cmviot RTM system.
 * <ul>
 *     <li>If the connection between the SDK and the Cmviot RTM system is interrupted because of network issues, the SDK triggers the {@link RTMEvents.status} callback and switches to the {@link RECONNECTING} state.</li>
 ...
 *     <li>If the app calls the {@link RTMClient.logout} method and the SDK successfully logs out of the Cmviot RTM system, the SDK triggers the {@link RTMEvents.status} callback and switches to the {@link DISCONNECTED} state.</li>
 * </ul>
 */
 | 'RECONNECTING'
/** @zh-cn
 * SDK 正在重新登录 Cmviot RTM 系统。
 *
 * <ul>
 *     <li>如果 SDK 重新登录 Cmviot RTM 系统成功，会触发回调 {@link RTMEvents.status}，并切换到 {@link CONNECTED} 状态。</li>
 *     <li>如果 SDK 重新登录 Cmviot RTM 系统失败，会保持  {@link RECONNECTING} 状态。</li>
 *     <li>如果登录被服务器拒绝，SDK 会触发回调 {@link RTMEvents.status}，并切换到 {@link FAILED} 状态。</li>
 * </ul>
 *
 */
/**
 * The SDK keeps logging in the Cmviot RTM system.
 * <ul>
 *     <li>If the SDK successfully logs in the Cmviot RTM system again, it triggers the {@link  RTMEvents.status} callback and switches to the {@link CONNECTED} state.</li>
 *     <li>If the SDK fails to log in the Cmviot RTM system again, the SDK stays in the {@link RECONNECTING} state. </li>
 ...
 */
 | 'CONNECTED'
/** @zh-cn
 * SDK 停止登录 Cmviot RTM 系统。
 *
 * <p>原因可能为：</p>
 * <p><ul>
 * <li>另一实例已经以同一用户 ID 登录 Cmviot RTM 系统。</li>
 * <li>token 已过期。</li></ul></p>
 * 请在调用方法 {@link RTMClient.logout} 后，调用方法 {@link RTMClient.login} 登录 Cmviot RTM 系统。</p>
 */
/**
 * The SDK gives up logging in the Cmviot RTM system, possibly because another instance has logged in the Cmviot RTM system with the same uid.
 *
 * <p>Call the {@link RTMClient.logout} method before calling the {@link RTMClient.login} method to log in the Cmviot RTM system again.</p>
 */
 | 'FAILED';
/** @zh-cn
 * 用户元数据订阅状态。
...
/** @zh-cn
 * Cmviot RTM 服务的限定区域。设置限定区域之后，Cmviot RTM SDK 只能连接位于限定区域的 Cmviot RTM 服务。
 */
/**
 * Region for the Cmviot RTM service. After setting a region, the Cmviot RTM SDK can only connect to the Cmviot RTM service in the specified region.
 */
...
declare class RTMStorage {
     ...
     * @param userId The userId of the user logging in the Cmviot RTM system.
     */
...
declare namespace RTMEvents {
    ...
    interface RTMClientEventMap {
        /** @zh-cn
         * 通知 SDK 与 Cmviot RTM 系统的连接状态发生了改变。
         * @event
         * @param connectionStatus.newState 新的连接状态
         * @param connectionStatus.reason 状态改变的原因
         */
        /**
         * Occurs when the connection state changes between the SDK and the Cmviot RTM system.
         * @event
...
declare class RTMClient extends EventBase<RTMEvents.RTMClientEventMap> {
    ...
    appId: string, 
    /**@zh-cn
     * 登录 Cmviot RTM 系统的用户 ID。该字符串不可超过 64 字节。以下为支持的字符集范围:<ul>
     ...
     */
    /**
     * The uid of the user logging in the Cmviot RTM system. The string length must be less than 64 bytes with the following character scope:<ul>
    ...
    /** @zh-cn
     * 用户登录 Cmviot RTM 系统。
     * @note 在 RTM 和 RTC 结合使用的场景下，Cmviot 推荐你错时进行登录 RTM 系统和加入 RTC 频道的操作。
     * @note 如果用户在不同的 RtmClient 实例中以相同用户 ID 登录，之前的登录将会失效，用户会被踢出之前加入的频道。
     * @return 该 Promise 会在登录成功后 resolve。
     */
    /**
     * Logs in to the Cmviot RTM system.
     *
     * @note If you use the Cmviot RTM SDK together with the Cmviot RTC SDK, Cmviot recommends that you avoid logging in to the RTM system and joining the RTC channel at the same time.
     ...
     * @return The Promise resolves after the user logs in to the Cmviot RTM system successfully.
     */
     ...
     */
    /**
     * Allows a user to log out of the Cmviot RTM system.
     *
     * After the user logs out of the Cmviot RTM system, the SDK disconnects from the Cmviot RTM system and destroys the corresponding event listener.
     * @return The Promises resolves after the user logs out of the Cmviot RTM system and disconnects from WebSocket.
     */
     ...
     * @param channelName The unique channel name of the Cmviot RTM stream channel. The string length must be less than 64 bytes with the following character scope:<ul>
     * <li>All lowercase English letters: a to z</li>
     * <li>All uppercase English letters: A to Z</li>
     * <li>All numeric characters: 0 to 9</li>
     * <li>The space character.</li>
...
declare interface ICmviotRTM {
    /** @zh-cn
     * Cmviot RTM SDK 的编译信息。
     */
    /**
     * Compilation information of the Cmviot RTM SDK.
     * @example `CmviotRTM.BUILD`
     */
    BUILD: string;
     ...
     */
    /**
     * Version of the Cmviot RTM SDK.
     * @example `CmviotRTM.VERSION`
     */
    VERSION: string;
    processId: string;
...
export { ChannelType, ConstantsType, ErrorInfo, GetChannelMetadataResponse, GetLockResponse, GetStateResponse, GetSubscribedUserListResponse, GetUserMetadataResponse, ICmviotRTM, LoginResponse, LogoutResponse, MetadataItem, MetadataOptions, RTMClient, RTMConfig, RTMEvents, RTMLock, RTMPresence, RTMStorage, RTMStreamChannel, RTMStreamChannelStatusCode, StateDetail, WhereNowResponse, WhoNowOptions, WhoNowResponse };
